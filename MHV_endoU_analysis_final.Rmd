...
Calculating "fold change" across all sites is an unbiased approach to identify potential nsp15 cleavage sites. An alternative approach, focuses on the sites of interest from the nsp15 substractive analysis and calculates fold change for those sites. 
...
new_changensp15 <- change_nsp15 %>% 
  ...
  arrange(desc(fold_change)) %>% 
  group_by(start, end, cell, time, viral_comp) %>%
  arrange(desc(fold_change)) %>%
  ungroup() %>%
  arrange(start) %>%
  group_by(cell, time) %>%
...
####Plotting > 4 log2foldchange sites 
...
dinuc_positions <- neg_table %>% 
  dplyr::select(start, end, dinuc)
B6_12_dinuc <- left_join(B6_12, dinuc_positions) %>% 
  unique() %>% 
  dplyr::select(start, end, log2foldchange, dinuc)
...
#### Table and plot of the top 15 sites (by cell, time, and virus) by normalized counts that are nsp15-dependent

```{r, message = F, warning = F}

top_nsp15 <- subtractive_nsp15 %>%
  group_by(cell, time, virus) %>%
  arrange(desc(normalized_count)) %>%
  do(head(., n = 15))
datatable(top_nsp15)

top_nsp15_graph <- ggplot(top_nsp15, aes(x = start, y = normalized_count,     color = cell)) +
    geom_bar(stat = "identity", position = 'dodge') + 
    scale_fill_brewer(palette="Set1") +
    facet_grid(virus ~ cell) +
    theme_cowplot() +
    labs(x="Position", y="normalized count") +
    theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
    scale_x_continuous(breaks=seq(0, 30000, 5000))
top_nsp15_graph 
...
#### Table of the top nsp15-dependent sites for RNase L cells infected with each virus type at all time points, filtered greatetst counts (top 100)

```{r, message = F, warning = F}

top_change_nsp15 <- neg_table %>% 
 inner_join(top_nsp15, by = c("start", "end")) %>%
 dplyr::select(start, end, dinuc, normalized_count.x, cell.x, virus.x,        time.x) %>%
 arrange(desc(normalized_count.x)) %>%
 unique()

RNaseL_top <- top_change_nsp15 %>% filter(cell.x == "RNaseL") %>% arrange(desc(normalized_count.x)) %>% head(100)
datatable(RNaseL_top)
 
```

#### Plots of the top nsp15-dependents sites after fold change analysis, filtered for the sites with greater than 2 log2fold change 

```{r, message = F, warning = F}

topfoldchange_nsp15 <- top_change_nsp15 %>% 
  dplyr::select(-dinuc) %>% 
  spread(virus.x, normalized_count.x)

topfoldchange_nsp15[is.na(topfoldchange_nsp15)] <- 0

topfoldchange_nsp15 <- topfoldchange_nsp15 %>% 
  mutate(MHVS_nsp15change = nsp15/MHVS) %>% 
  mutate(MHVV_nsp15change = nsp15/MHVV) %>% 
  mutate(ns2_nsp15change = nsp15/ns2) %>% 
  mutate(mock_nsp15change = nsp15/mock) %>% 
  gather(virus.x, fold_change, MHVS_nsp15change:mock_nsp15change) %>%
  filter(fold_change > 0 & fold_change <= 1) %>%
  arrange(fold_change) %>%
  mutate(log2foldchange = -log2(fold_change))

topfoldchange_graph <- filter(topfoldchange_nsp15, log2foldchange >= 2) %>%
  ggplot(aes(x = start, y = log2foldchange, color = cell.x)) +
  geom_bar(stat = "identity", position = 'dodge') + 
  scale_fill_brewer(palette="Set1") +
  facet_grid(virus.x ~ cell.x) +
  theme_cowplot() +
  labs(x="Position", y="Log2foldchange") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
  #scale_x_continuous(breaks=seq(0, 30000, 5000))
